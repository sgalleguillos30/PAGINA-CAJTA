...
use App\Http\Controllers\InfoLeyes\LeyKarinController;
use App\Http\Controllers\GeneroController;
use App\Http\Controllers\ValidaCertificados\ValidaSCPController;
use App\Http\Controllers\ValidaCertificados\ValidaBAJController;
...
Route::group(['prefix' => 'nosotros/'], function () {
    ...
    Route::group(['prefix' => 'postulantes/'], function() {
        ...
        Route::post('/solicitud', 'NosotrosController@postulantes_post_solicitud')->name('nosotros.postulantes.postsolicitud');
    });
});
...
Route::group(['prefix' => 'genero/'], function () {
    Route::get('/', 'InfoGenero\GeneroController@index')->name('genero.index');
    Route::get('/denuncia', 'InfoGenero\GeneroController@denuncia')->name('genero.denuncia');
});
...
Route::prefix('ley-karin/')
        ->name('infoleyes.leykarin.')
        ->group( function() {
            Route::get('/', [LeyKarinController::class, 'index'])->name('index');
            Route::get('/normativa-legal', [LeyKarinController::class, 'normativa'])->name('normativa');
        });


Route::prefix('valida-documentos/')
    ->name('valida-documentos.')
    ->group( function() {
        
        Route::prefix('scp/')
            ->name('scp.')
            ->group( function() {
                Route::get('/', [ValidaSCPController::class, 'index'])->name('index');
                Route::get('/{solicitud}/{uuid}', [ValidaSCPController::class, 'validacion'])->name('validacion');
            });

        Route::prefix('baj/')
                ->name('baj.')
                ->group( function () {
                    Route::get('/{baj}/{uuid}', [ValidaBAJController::class, 'validacion'])->name('validacion');
                });
